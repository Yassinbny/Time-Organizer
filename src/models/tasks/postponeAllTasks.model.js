...
export default async function postponeAllTasksModel (daysToPostpone=0, hoursToPostpone=0) {
    try {
     ...
     let updateCount = 0;
        ...
        const [result] = await pool.query (`UPDATE tasks SET finish_on = ? WHERE task_id = ?`, [newFinishOnString, task.task_id]);
        ...
        //};
        return {
            message: updateCount > 0 ? "Tareas pospuestas con éxito." : "No se realizaron cambios."
        }
